...
class DeformableDETRMaskFusionHead(BasePanopticFusionHead):
    ...
    def panoptic_postprocess(self, mask_cls, mask_pred):
        ...
        qi2id = defaultdict(int) # query index to panoptic id
        id2qi = defaultdict(int) # panoptic id to query indices
        id2np = defaultdict(int) # panoptic id to number of pixels
            ...
            for k in range(cur_classes.shape[0]):
                ...
                if mask_area > 0 and original_area > 0:
                    ...
                    if not isthing:
                        ...
                        qi2id[keep_ind[k]] = pred_class
                        # keep the query that generates largest stuff region
                        if mask_area > id2np[pred_class]:
                            id2qi[pred_class] = keep_ind[k]
                    else:
                        panoptic_id = pred_class + instance_id * INSTANCE_OFFSET
                        panoptic_seg[mask] = panoptic_id
                        qi2id[keep_ind[k]] = panoptic_id
                        if mask_area > id2np[panoptic_id]:
                            id2qi[panoptic_id] = keep_ind[k]
                        instance_id += 1

        panoptic_seg = panoptic_seg.detach().cpu().numpy()
        pan_labels = list(set(qi2id.values()))
        pan_masks = np.array([panoptic_seg == pan_label for pan_label in pan_labels])
        qis = [id2qi[pan_label] for pan_label in pan_labels]
        # NOTE: values: intance_id -> label_index
        qi2id = {q : pan_labels.index(_) for q, _ in qi2id.items()}

        return panoptic_seg, pan_masks, pan_labels, qi2id, qis
