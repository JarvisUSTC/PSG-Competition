...
class BoxMaskHungarianAssigner(BaseAssigner):
    ...
    def assign(self,
        ...
        with torch.no_grad():
            ...
            #         (mask_pred.shape[-2] // 2, mask_pred.shape[-1] // 2),
            #         mode='bilinear',
            #         align_corners=False).squeeze(1)
            #     gt_mask = F.interpolate(
            #         gt_mask.unsqueeze(1),
            #         (gt_mask.shape[-2] // 2, gt_mask.shape[-1] // 2),
            #         mode='bilinear',
            #         align_corners=False).squeeze(1)
            if self.mask_cost.weight != 0:
                mask_cost = self.mask_cost(mask_pred.reshape(num_query,-1), gt_mask.reshape(num_gt,-1))
            else:
                mask_cost = 0
            ...
            if stuff_label is not None:
                if (gt_labels < stuff_label).sum() > 0:
                    iou_cost[:, gt_labels > stuff_label] = iou_cost[:, gt_labels < stuff_label].mean()
                    reg_cost[:, gt_labels > stuff_label] = reg_cost[:, gt_labels < stuff_label].mean()
                else:
                    iou_cost[:, gt_labels > stuff_label] = 0
                    reg_cost[:, gt_labels > stuff_label] = 0

            cost = cls_cost + mask_cost + dice_cost + iou_cost + reg_cost
