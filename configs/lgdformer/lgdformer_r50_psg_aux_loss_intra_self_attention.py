...
custom_imports = dict(imports=[
    ...
    'openpsg.models.relation_heads.lgdformer_head',
    'openpsg.datasets',
...
model = dict(bbox_head=dict(
    ...
    predicate_node_generator=dict(
        num_rel_query=100,
        num_classes=len(object_classes),
        intra_self_attention=dict(type='MultiheadAttention',
                    embed_dims=256,
                    num_heads=8,
                    dropout=0.1),
        update_query_by_rel_hs=True,
    ),
), )
...
# dataset_config = dict(cache=f'./work_dirs/{expt_name}/cache')
